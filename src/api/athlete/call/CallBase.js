...
export default class CallBase {
    ...
    //   else if (response.headers.get('content-type') === 'application/x-ndjson') {
    //     return response.text().then(rowData => {
    //       const rowJsons = rowData.split('}\n');
    //       const jsons = [];
    //       rowJsons.forEach((rowJson, index) => {
    //         if (rowJson.trim() !== '') {
    //           const objJSON = index === rowJsons.length - 1 ? rowJson : `${rowJson }}`;
    //           jsons.push(JSON.parse(objJSON));
    //         }
    //       });
    //       const value = CallBase._formatJson(jsons, response.status);
    //       return value;
    //     });
    //   }
      else {
